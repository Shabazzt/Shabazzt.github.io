const sortByTriggerBtn = document.getElementById('sortByTrigger');
const sortByOptionsArr = ['Бестселлерам', 'Названию', 'Пользовательскому рейтингу', 'Дате добавления', 'Популярности', 'Сначала старые', 'Сначала новые', ];

function createSortByDropMenu() {
    let ul = document.createElement('ul');
    ul.className = 'dropDownMenu';
    ul.id = 'sortByDropDownMenu';
    for (let i = 0; i < sortByOptionsArr.length; i++) {
        ...
        <span class="optionName">${sortByOptionsArr[i]}</span>
    </label>`;
        ul.appendChild(li);
    }
    sortByTriggerBtn.appendChild(ul);
}
createSortByDropMenu();